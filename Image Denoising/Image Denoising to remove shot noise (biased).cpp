...
//sd=Standard Deviation of Gaussian Filter
	int sd=25;
...
//Applying Gaussian filter to Imagedatatransformed which is stored in the boundary extended Imagedata
double wijkl;
...
for(int i=0;i<Size;i++){
    for(int j=0;j<Size;j++){
        Imagedataoutput[i][j][0]= ((2*(pow((int)Imagedatafiltered[i][j][0],2)))-3)/8;
    }
}
...
psnr= 10* log10((pow(max,2))/mse);
	...
	fwrite(Imagedatafiltered, sizeof(unsigned char), Size*Size*BytesPerPixel, file);
	fclose(file);
