...
//Initializing arrays that would take all the values in three color channels in the window
	int Median0[N*N];
	int Median1[N*N];
	int Median2[N*N];
...
//Assigning a Boundary extended matrix
    ...
    unsigned char Imagedataoutputmed[Size][Size][BytesPerPixel];
    unsigned char Imagedataoutputgauss[Size][Size][BytesPerPixel];
...
for(int i=((N-1)/2); i<(Size+(N-(((N-1)/2)+1)));i++){
    for(int j=((N-1)/2);j<(Size+(N-(((N-1)/2)+1)));j++){
        ...
        Imagedataoutputgauss[i-((N-1)/2)][j-((N-1)/2)][0]=(int)(sum0/tot);
        Imagedataoutputgauss[i-((N-1)/2)][j-((N-1)/2)][1]=(int)(sum1/tot);
        Imagedataoutputgauss[i-((N-1)/2)][j-((N-1)/2)][2]=(int)(sum2/tot);
    }
}
//Median Filter
 ...
 for(int i=((N-1)/2); i<(Size+(N-(((N-1)/2)+1)));i++){
    for(int j=((N-1)/2);j<(Size+(N-(((N-1)/2)+1)));j++){
            int p=0;
            Median0[p]={0}; Median1[p]={0}; Median2[p]={0};
            for(int x= (i-((N-1)/2)); x<=(i+((N-1)/2));x++ ){

                for(int y = (j-((N-1)/2)); y<=(j+((N-1)/2));y++){

                    Median0[p]=Imagedataoutputgauss[x][y][0];
                    Median1[p]=Imagedataoutputgauss[x][y][1];
                    Median2[p]=Imagedataoutputgauss[x][y][2];
                    p++;
                }
                }
            int temp=0;
    ...
    for(int i=0;i<(N*N);i++)
	{
        ...
        Imagedataoutputmed[i-((N-1)/2)][j-((N-1)/2)][0]=(Median0[(N-1)/2]);
        Imagedataoutputmed[i-((N-1)/2)][j-((N-1)/2)][1]=(Median1[(N-1)/2]);
        Imagedataoutputmed[i-((N-1)/2)][j-((N-1)/2)][2]=(Median2[(N-1)/2]);
...
int totmse=0;
long int max=(pow(2,24)-1);
double psnr;
double mse;
for(int i=0;i<Size;i++){
    for(int j=0;j<Size;j++){
        mse=(pow((Imagedataoutputmed[i][j][0]-Imagedatainput[i][j][0]),2))/(pow(Size,2));
        totmse= totmse+mse;
    }
}
psnr= 10* log10((pow(max,2))/mse);
	...
	fwrite(Imagedataoutputmed, sizeof(unsigned char), Size*Size*BytesPerPixel, file);
	fclose(file);
