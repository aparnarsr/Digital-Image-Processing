...

{
	...
	int N=7;
	//unsigned char W[Size][Size];
	//unsigned char wijkl[Size][Size];
...
for(int i=((N-1)/2); i<(Size+(N-(((N-1)/2)+1)));i++){
    for(int j=((N-1)/2);j<(Size+(N-(((N-1)/2)+1)));j++){
            ...
            int sum0=0; int sum1=0; int sum2=0;
            for(int x= (i-((N-1)/2)); x<=(i+((N-1)/2));x++ ){
                for(int y = (j-((N-1)/2)); y<=(j+((N-1)/2));y++){

                    sum0=sum0+(int)Imagedata[x][y][0];
                    sum1=sum1+(int)Imagedata[x][y][1];
                    sum2=sum2+(int)Imagedata[x][y][2];

                }
          }

        Imagedataoutput[i-((N-1)/2)][j-((N-1)/2)][0]=(int)(sum0/den);
        Imagedataoutput[i-((N-1)/2)][j-((N-1)/2)][1]=(int)(sum1/den);
        Imagedataoutput[i-((N-1)/2)][j-((N-1)/2)][2]=(int)(sum2/den);
    }
}
